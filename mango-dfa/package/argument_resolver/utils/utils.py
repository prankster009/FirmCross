...
def extract_lua_parameters(c_decompile_code):
    """从C代码中提取Lua函数的特定参数并区分字符串和常量"""
    
    # 提取luaL_loadfile的第二个参数
    loadfile_pattern = r'luaL_loadfile\s*\(\s*L\s*,\s*(".*?"|[A-Za-z_][A-Za-z0-9_]*)\s*\)'
    loadfile_match = re.search(loadfile_pattern, c_decompile_code)
    if loadfile_match:
        lua_file = loadfile_match.group(1)
        if lua_file.startswith('"'):
            # lua_file is a string
            lua_file = lua_file[1:-1]
    
            # 提取lua_getglobal的第二个参数
            getglobal_pattern = r'lua_getglobal\s*\(\s*L\s*,\s*(".*?"|[A-Za-z_][A-Za-z0-9_]*)\s*\)'
            getglobal_match = re.search(getglobal_pattern, c_decompile_code)
            if getglobal_match:
                lua_func = getglobal_match.group(1)
                if lua_func.startswith('"'):
                    # lua_func is a string
                    lua_func = lua_func[1:-1]

                    tainted_idxs = list()
                    # 提取所有lua_pushstring的第二个参数
                    pushstring_pattern = r'lua_pushstring\s*\(\s*L\s*,\s*(".*?"|[A-Za-z_][A-Za-z0-9_]*)\s*\)'
                    for idx, match in enumerate(re.finditer(pushstring_pattern, c_decompile_code)):
                        param = match.group(1)
                        if not param.startswith('"'):
                            # param is a variable, and should be tainted
                            tainted_idxs.append(idx)
                    if tainted_idxs:
                        return (os.path.basename(lua_file), lua_func, tainted_idxs)
    return None
...
class Utils:
    ...
    def arguments_from_function(function: Function):
        ...
        if function.name == "sprintf" and len(arguments)==2:
            platform_name = "Linux"
            reg_name = DEFAULT_CC[cc.arch.name][platform_name].ARG_REGS[2]
            third_reg = SimRegArg(reg_name, cc.arch.bytes)
            arguments.append(third_reg)
