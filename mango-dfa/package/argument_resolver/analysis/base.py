...
# from argument_resolver.external_function.function_declarations import CUSTOM_DECLS, get_CUSTOM_DECLS
from argument_resolver.external_function.function_declarations import get_CUSTOM_DECLS
...
class ScriptBase:
    ...
    def init_analysis(self, workers: int, show_progress_bar: bool, ld_paths: list):
        ...
        if self.category == "lua_register":
            cfg = project.analyses.CFGFast(
                normalize=True, data_references=True, show_progressbar=show_progress_bar,
                resolve_indirect_jumps=True   
            )
            lib_name = os.path.basename(self.bin_path)[:-3]
            luaopen_wrapper_name = f"luaopen_{lib_name}"
            wrapper_addr = None
            # set_trace()
            for func_addr, func in project.kb.functions.items():
                func_name = func.name if hasattr(func, 'name') else f"unnamed_{func_addr:x}"
                if func_name == luaopen_wrapper_name:
                    wrapper_addr = func.addr
                
            if wrapper_addr:
                # set_trace()
                entries = [wrapper_addr]
                # print("add emulated")
                # 创建CFGEmulated并指定多入口
                cfg = project.analyses.CFGEmulated(
                    starts=entries,
                    call_depth=3, # only trace for luaL_register func
                    keep_state=True,
                    context_sensitivity_level=2,    # 中等级别上下文敏感（默认1，最高2）
                    enable_symbolic_back_traversal=False,  # 关闭符号执行辅助（提速）
                    enable_advanced_backward_slicing=True, # 关闭逆向切片（提速）
                    resolve_indirect_jumps=True,
                )
            
        else:
            cfg = project.analyses.CFGEmulated(
                context_sensitivity_level=1,    # 中等级别上下文敏感（默认1，最高2）
                enable_symbolic_back_traversal=False,  # 关闭符号执行辅助（提速）
                enable_advanced_backward_slicing=False, # 关闭逆向切片（提速）
                resolve_indirect_jumps=True,
    ...
    def rename_form_param_parser(self):
        ...
        LIBRARY_DECLS = get_LIBRARY_DECLS()
        all_funcs = [
            x
            for x in all_funcs
    ...
    def load_sinks(custom_sink=None, arg_pos=None, category=None) -> List[Sink]:
        # set_trace()
        sinks = []
        if custom_sink and arg_pos is not None:
