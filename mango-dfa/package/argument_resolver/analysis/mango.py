...
# from typing import Any, Dict, Generator, List, Optional, , Tuple, NamedTuple
import networkx
import concurrent.futures

from argument_resolver.analysis.get_lua_register import get_lua_register_table
import angr
...
from argument_resolver.utils.utils import Utils, extract_lua_parameters
...
from argument_resolver.external_function.function_declarations import read_lua2c_API_info_and_update_CUSTOM_DECLS
...
class MangoAnalysis(ScriptBase):
    def __init__(self, *args, **kwargs):
        ...
        self.table_func_path = kwargs.pop("table_func_path")
        self.C2Lua_API_path = kwargs.pop("C2Lua_API_path")
        self.Lua2C_API_path = kwargs.pop("Lua2C_API_path")
        if self.Lua2C_API_path:
            read_lua2c_API_info_and_update_CUSTOM_DECLS(self.Lua2C_API_path)
    ...
    def get_lua_register_func(self):
        pass

    def get_C2Lua_API_info(self):
        # set_trace()
        for sink, sink_function in self.get_sink_callsites(self.sinks):
            ...
            for depth in range(1, self.max_depth + 1):
                sub_traces: Set[CallTrace] = traces_to_sink(
                    ...
                    depth,
                    self.excluded_functions[sink_function],
                )
            ...
            C2Lua_API_records = list()
            for trace in traces:
                if trace.callsites:
                    calliste_related_to_sink = trace.callsites[0]
                    caller_addr = calliste_related_to_sink.caller_func_addr
                    function = self.project.kb.functions[caller_addr]
                    if not function.normalized:
                        function.normalize()

                    self.project.analyses.VariableRecoveryFast(function)
                    cfg = self.project.analyses.CFGFast(normalize=True)
                    cc_analysis = self.project.analyses.CallingConvention(function, cfg=cfg, analyze_callsites=True)
                    
                    # 应用分析结果到函数对象
                    if cc_analysis.cc is not None:
                        # print(f"识别的调用约定: {cc_analysis.cc}")
                        function.calling_convention = cc_analysis.cc
                        function.prototype = cc_analysis.prototype

                    decompiler = self.project.analyses.Decompiler(function, cfg=cfg)

                    if decompiler.codegen:
                        # print(decompiler.codegen.text)
                        decompile_c_code = decompiler.codegen.text
                        # c_code = """
                        # int main() {
                        ...
                        # """
                        # set_trace()
                        API_info = extract_lua_parameters(decompile_c_code)
                        if API_info:
                            lua_file, lua_func, tainted_idxs = API_info
                            record = {
                                "file": lua_file,
                                "func": lua_func,
                                "idxs": tainted_idxs
                            }
                            C2Lua_API_records.append(record)
            # set_trace()
            if C2Lua_API_records:
                C2Lua_API_path = self.C2Lua_API_path
                if C2Lua_API_path:
                    if not os.path.exists(os.path.dirname(C2Lua_API_path)):
                        os.makedirs(os.path.dirname(C2Lua_API_path), exist_ok=True)
                    with open(C2Lua_API_path, "w") as f:
                        json.dump(C2Lua_API_records, f, indent=4)

    def find_lua_register_table(self):
        for sink, sink_function in self.get_sink_callsites(self.sinks):
            cfg = self.project.kb.cfgs.get_most_accurate()
            ...
            for idx, trace in enumerate(
                ...
                for call_idx, callsite in enumerate(trace.callsites):
                    ...
                    # set_trace()
                    for store_func in handler.white_list:
                        if store_func.name == sink_function.name:
                            target_table_atom = sink_atoms[0]
                            target_mv = store_func.arg_vals[target_table_atom]
                            table_addr = 0
                            for offset, value_set in sorted(target_mv.items(), key=lambda x: x[0]):
                                for value in value_set:
                                    if value.concrete:
                                        table_addr = value.args[0]
                            if table_addr:
                                ...
                                if self.project.arch.bits == 32:
                                    ...
                                    raise ValueError("Unsupported architecture bits.")

                                # 初始化一个空列表来存储函数名和地址
                                functions = get_lua_register_table(self.project, table_addr, addr_size)
                                # 打印所有的函数名和地址
                                for name, addr in functions:
                                    print(f"Function Name: {name}, Address: 0x{addr:x}")
...
def get_cli_args():
    ...
    # 新增参数: table_func_path
    run_group.add_argument(
        "--table-func-path",
        dest="table_func_path",
        default="",
        type=str,
        help="Path to the save info related to Lua table function(string, default: '')",
    )
    ...
    run_group.add_argument(
        ...
        default="",
        type=str,
        help="Path to the save info related to C2Lua API info(string, default: '')",
    )
    ...
    run_group.add_argument(
        ...
        default="",
        type=str,
        help="Path to the read info related to Lua2C API info(string, default: '')",
    )
...
def main():
    ...
    args = get_cli_args()
    if args.merge:
        args.__dict__.pop("C2Lua_API_path")
        args.__dict__.pop("table_func_path")
        args.__dict__.pop("Lua2C_API_path")
        ...
        if args.sink_category=="lua_register":
            analyzer.find_lua_register_table()
        elif args.sink_category=="c2lua_api":
            analyzer.get_C2Lua_API_info()
        else:
            analyzer.analyze()
