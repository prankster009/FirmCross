...
class UnistdHandlers(HandlerBase):
    ...
    def handle_uf_socket_msg_read(
        ...
        """
        modified from read
        Process uf_socket_msg_read and marks it as taint
        .. sourcecode:: c
            size_t uf_socket_msg_read(int a1, _QWORD *a2);
        :param ReachingDefinitionsState state:    reaching definitions state
        :param Codeloc codeloc:              Code location of the call
        :param handler_name:                 Name of function to handle
        """
        ...
        self.log.debug("RDA: %s(), ins_addr=%#x", stored_func.name, stored_func.code_loc.ins_addr)
        # set_trace()
        cc = self._calling_convention_resolver.get_cc("uf_socket_msg_read")
        ...
        for ptr in Utils.get_values_from_multivalues(buf_ptrs):
            ...
            if Utils.is_stack_address(ptr):
                sp_offset = SpOffset(state.arch.bits, state.get_stack_offset(ptr))
                if sp_offset.offset is None:
                    continue
                memloc = MemoryLocation(sp_offset, state.arch.bytes, endness=Endness.BE)
            elif Utils.is_heap_address(ptr):
                heap_addr = Utils.get_heap_offset(ptr)
                heap_address = None
                for heap in state.heap_allocator.allocated_addresses:
                    if heap.value == heap_addr:
                        heap_address = heap
                        break
                    elif heap.value <= heap_addr:
                        heap_address = heap + (heap_addr-heap.value)
                        break
                if heap_address is None:
                    continue
                memloc = MemoryLocation(heap_address, state.arch.bytes, endness=Endness.BE)
            else:
                # allcate a heap 0x90
                pointer_heap_addr = state.heap_allocator.allocate(0x90)
                # get the memory space of state.arch.bytes
                memloc = MemoryLocation(pointer_heap_addr, state.arch.bytes, endness=Endness.BE)
                pointer_heap_mv = MultiValues(claripy.BVV(pointer_heap_addr.value, state.arch.bits))
                change_arg = False
                if isinstance(arg_buf, SimRegArg):
                    reg_offset = state.arch.registers[arg_buf.reg_name][0]  # 获取寄存器偏移
                    for atoms in stored_func.args_atoms:
                        for reg in atoms:
                            if reg.reg_offset == reg_offset:
                                stored_func.arg_vals[reg] = pointer_heap_mv
                                state.registers.store(reg_offset, pointer_heap_mv, size=state.arch.bytes)
                                change_arg = True
                if not change_arg:
                    continue
            ...
            resolved = False
            for func_effect in stored_func._data.effects:
                if func_effect.dest == memloc:
                    resolved = True
                    break
            
            if resolved:
                # this mean the func has already analyzed
                continue

            # construct the msg_buf
            alloc_size = 0x200
            msg_heap_addr = state.heap_allocator.allocate(alloc_size)
            msg_heap_memloc = MemoryLocation(msg_heap_addr, alloc_size)
            msg_bvs = claripy.BVS(
                f"{stored_func.name}({parent_name})_msg_buffer@0x{stored_func.code_loc.ins_addr:x}",
                alloc_size * 8)
            msg_bvs.variables = frozenset(set(msg_bvs.variables) | {"TOP"})
            msg_mv = MultiValues(msg_bvs)
            self.fd_tracker[stored_func.name].append(
                {"val": msg_bvs, "parent": parent_fds, "ins_addr": stored_func.code_loc.ins_addr})
            stored_func.depends(msg_heap_memloc, *stored_func.atoms, value=msg_mv)
            
            # store the msg_buf in the pointer
            heap_ptr_mv = MultiValues(claripy.BVV(msg_heap_addr.value, state.arch.bits))
            # the memloc space must equal to value length
            stored_func.depends(memloc, *stored_func.atoms, value=heap_ptr_mv)
        # set_trace()
        return True, state, MultiValues(claripy.BVV(0x200, state.arch.bits))


    @HandlerBase.returns
    @HandlerBase.tag_parameter_definitions
    def handle_uf_socket_msg_read_bak(
        self,
        state: "ReachingDefinitionsState",
        stored_func: StoredFunction,
    ):
        ...
        for ptr in Utils.get_values_from_multivalues(buf_ptrs):
            ...
            memloc = MemoryLocation(sp_offset, self.MAX_READ_SIZE, endness=Endness.BE)
