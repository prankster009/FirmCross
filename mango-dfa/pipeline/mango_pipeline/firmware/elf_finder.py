...
class FirmwareFinder:
    ...
    def find_elf_files(root_dir: Path, progress, exclude_libs=True):
        ...
        elfs = list()
        for x in output.decode().split("\n"):
            # TODO: use file to determine the ELF type is inaccurate, because old version 'file' cmd would mistake the ELF type, such as portscan in xiaomi
            # TODO: we need to fix it.
            if "ELF" in x and (not exclude_libs or "shared object" not in x or "/portscan" in x):
                elfs.append(x)
