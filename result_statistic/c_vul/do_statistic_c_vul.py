...
def create_excel_table(rows, brand_data, show_firm):
    columns = [
        "Name", "# Firm", "Binaries", "Alerted Bins", "TruPoC Bins",
        "Binaries Resolved", "No Sinks", "Alerts", "TruPoCs",
        "Error", "OOMKilled", "Timeout", "Analysis Time",
        "AVG Time", "AVG Bin Time"
    ]
    data = []
    found_start = False
    ...
    # 创建 ExcelWriter 对象
    with pd.ExcelWriter('MANGO_RESULTS.xlsx', engine='openpyxl') as writer:
        df.to_excel(writer, sheet_name='MANGO RESULTS', index=False)
...
def show_table(results: dict, show_firm):
    ...
    create_excel_table(rows, brand_data, show_firm)
    print(f"Alerts: {rows[-1][7+offset]}")
    print(f"AVG Time: {rows[-1][12+offset]}")
    Console().print(table)

def get_alerts_and_time(results: dict, show_firm):
    """Make a new table."""
    rows = []
    ...
    for idx, x in enumerate(rows[:-1]):
        if show_firm and not found_start and idx+1 < len(rows) and rows[idx+1][0] in brand_data:
            ...
            table.add_row(*x[:-2], x[-1])
    Alerts = rows[-1][7+offset]
    AVG_Time = rows[-1][12+offset]
    return Alerts, AVG_Time
...
if __name__ == '__main__':
    ...
    # set_trace()
    Alerts, AVG_Time = get_alerts_and_time(results, show_firm=args.show_firmware)
    print(Alerts, AVG_Time)
    # python do_statistic_c_vul.py /data/lrh/first_work/firmcross_ae/minimize_testcase/result_one_xiaomi_fw/single_c/
